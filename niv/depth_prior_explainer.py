...
def annotated_forward(model, left_t, right_t, rs_disp_t, conf_t, num_iters=8):
    ...
    with torch.no_grad():
        ...
        from core_niv.niv.smart_lite.model import build_gwc_volume
...
def main():
    ...
    p.add_argument("--ckpt", required=True, help="Path to stage3_best.pt")
    p.add_argument("--ffs-ckpt", default=None,
                   help="FFS backbone weights (default: weights/model_best_bp2_serialize.pth)")
    ...
    ffs_ckpt = args.ffs_ckpt or os.path.join(_FFS_ROOT, "weights", "model_best_bp2_serialize.pth")

    # Load model
    from core_niv.niv.smart_lite.ablation.d1_models import build_d1_model
    model = build_d1_model("edgenext", max_disp=64, num_iters=8, ffs_ckpt=ffs_ckpt)
    ...
    if args.left and args.right:
        ...
        depth_mm = _load_depth_mm(args.depth) if args.depth else \
                   np.zeros((stream_h, stream_w), dtype=np.uint16)
        print(f"Input: {stream_w}×{stream_h}  focal={focal_px:.1f}px  baseline={baseline_m*1000:.1f}mm  "
              f"depth valid={100*(depth_mm>0).mean():.1f}%")
    else:
        _sample_dir  = os.path.join(_FFS_ROOT, "sample")
        ...
        if os.path.isfile(_sample_l) and os.path.isfile(_sample_r):
            ...
            depth_mm  = _load_depth_mm(_sample_d) if os.path.isfile(_sample_d) else \
                        np.zeros(left_bgr.shape[:2], dtype=np.uint16)
            ...
            if args.output_dir is None:
                args.output_dir = os.path.join(_FFS_ROOT, "sample_output")
